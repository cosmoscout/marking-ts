...
import {
  ClickState, DragState, Groups, ItemState, MenuItemEventType, SettingsGroup,
} from '../enums';
...
export default class MenuItem extends Base implements MenuIdentifier {
  ...
  public init(): void {
    ...
    this._inputAngle$ = this.menu.inputPosition$.pipe(
      ...
      distinctUntilChanged(
        ...
        (previousAngle: number, currentAngle: number): boolean => Math.round(Angle.toDeg(previousAngle)) === Math.round(Angle.toDeg(currentAngle)),
      ),
    );
  ...
  protected setupGeometry(): void {
    ...
    if (this.childCount === 0 && this.settings[SettingsGroup.GEOMETRY].useActionShape) {
      ...
      this.geometry.segments[2].point = this.geometry.segments[2].point.add(
        new paper.Point(
          this.settings[SettingsGroup.GEOMETRY].size / 3,
          0,
        ),
      );
  ...
  protected setupIcon(): void {
    if (typeof this.iconName === 'undefined' || this.iconName === '') {
      ...
      if (typeof icon === 'undefined') {
        ...
        this.icon.scale(
          this.settings[SettingsGroup.SCALES].icon.base * this.settings[SettingsGroup.SCALES].icon.solo,
        ); // Default Size = 512px   1/16 = 0.0625 = 32px
  ...
  protected getChildren(): Array<MenuItem> {
    return <Array<MenuItem>>(<paper.Item[]> this.children)
      .filter((child: paper.Item): boolean => child instanceof MenuItem);
  }
  ...
  protected selectionLogic(angle: number): void {
    ...
    if (!this.isRoot) {
      this.isInBackNavigationArc = Angle.between(
        angle,
        (<ArcDefinition> this.parentArc).from,
        (<ArcDefinition> this.parentArc).to,
      );
    }
  ...
  protected clickLogic(clickState: ClickState): void {
    ...
    if (dist >= this.settings[SettingsGroup.RADII].maxClickRadius
        && this.settings[SettingsGroup.MAIN].enableMaxClickRadius) {
  ...
  protected traceLogic(decisionPoint: paper.Point): void {
    ...
    if (!this.isRoot
        && this.isInBackNavigationArc
        && CENTER.getDistance(localPos) >= this.settings[SettingsGroup.MAIN].minTraceDistance) {
      this.connector.visible = false;
      this.navigateBack();
      return;
    }
  ...
  protected dragLogic(drag: DragDefinition): void {
    ...
    if (drag.state === DragState.END
        && localPos.getDistance(CENTER) > this.settings[SettingsGroup.MAIN].minTraceDistance) {
      this.changeActive();
      return;
    }
    ...
    if (CENTER.getDistance(localPos) >= this.settings[SettingsGroup.MAIN].minTraceDistance
        && !this.isInBackNavigationArc) {
  ...
  protected animateStateHidden(): void {
    ...
    if (this.isRoot) {
      ...
      to = {
        scaling: 0,
        position: CENTER.subtract(<paper.Point> this.position)
          .add((<paper.Point> this.position)
            .multiply(this.settings[SettingsGroup.SCALES].dot)),
      };
    }
  ...
  protected animateStateBack(): void {
    ...
    if (!parent.isRoot) {
      const parentParent = (<MenuItem>(<MenuItem> this.parent).parent);

      rootPos = (<paper.Point> this.root.position).add(parent.globalToLocal(this.menu.inputPosition)).floor();
      parentParent.connector.strokeWidth = this.settings[SettingsGroup.CONNECTOR].width;
      parentParent.connector.strokeColor = ColorFactory.fromString(this.settings[SettingsGroup.CONNECTOR].color);
      parentParent.geometry.opacity = 1;
      parentParent.getChildren().forEach((child: MenuItem) => {
        child.geometry.opacity = 1;
      });
    }
  ...
  protected animateStateSubmenu(): void {
    ...
    const itemPos = REFERENCE_POINT.clone();
    ...
    const parentDelta = localMousePos.subtract(itemPos);
