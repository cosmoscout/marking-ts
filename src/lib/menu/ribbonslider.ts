...
import {
  ClickState, DragState, ItemState, MenuItemEventType, SettingsGroup,
} from '../enums';
...
export default class Ribbonslider extends MenuItem {
  ...
  protected dragLogic(drag: DragDefinition): void {
    ...
    const range = (this.configuration.max - this.configuration.min) / this.configuration.precision;

    let value = this.configuration.precision * Math.round(percentage * range) + this.configuration.min;
  ...
  protected setGeometryColorSelected(): void {
    (<paper.Path> this.childIndicator).fillColor = ColorFactory.fromString(
      this.settings[SettingsGroup.GEOMETRY].selectionColor,
    );
  }
  ...
  private setupRibbonContent(): void {
    ...
    for (let i = 0; i <= this.getRibbonLength() / (this.configuration.stepDist / 2); i += 1) {
      ...
      if (i % 2 === 0) {
        const valueText = <paper.PointText> this.text.clone();
        if (this.inSnappingRange()) {
          valueText.rotate(90);
        }

        const stepPrecision = this.getPrecision() / 2;

        const stepValue = Math.round((this.configuration.min + i * stepPrecision) * 100) / 100;
        ...
        const dots = this.grabDot.place(position);
  ...
  private setupGradient(): void {
    ...
    const gradient = <paper.Gradient>ColorFactory.fromArray([
  ...
  private setupGrabDotSymbol(): void {
    const grabDotGroup = new paper.Group();
    const dots = Array<paper.Item>();

    const circle = new paper.Path.Circle(ZERO_POINT, 3);
  ...
  private getPrecision(): number {
    return (typeof this.configuration.stepSize !== 'undefined')
      ? this.configuration.stepSize
  ...
  private updateRibbonPosition(delta: number, animated: boolean = false): void {
    ...
    if (delta >= 0) {
      const leftCenterPos = this.ribbonGroup.bounds.leftCenter.x
          + (this.settings[SettingsGroup.RIBBONSLIDER].gradientLength / 2)
          + delta;
      if (leftCenterPos <= 0) {
      ...
      const rightCenterPos = this.ribbonGroup.bounds.rightCenter.x
          - (this.settings[SettingsGroup.RIBBONSLIDER].gradientLength / 2)
          + delta;
      if (rightCenterPos >= 0) {
        positionX = this.ribbonGroup.position.x + delta;
